...
function PipelineDetails() {
  ...
  const filteredCoEvolution = evolutionData.filter((item) => {
    const matchesFilter =
      ...
      (coEvolutionFilter === "Detected Co-evolution" &&
        item.code_co_evolution) ||
  ...
  const indexOfFirstCoEvolution =
    indexOfLastCoEvolution - CO_EVOLUTION_PER_PAGE;
  const currentCoEvolution = filteredCoEvolution.slice(
    indexOfFirstCoEvolution,
    indexOfLastCoEvolution
  );

  const coEvolutionTotalPages = Math.ceil(
    filteredCoEvolution.length / CO_EVOLUTION_PER_PAGE
  );
  ...
  const filteredEvolution = evolutionData.filter((item) => {
    const matchesFilter =
      filterEvolution === "All" ||
      (filterEvolution === "Production Files Added" &&
        item.p_status_evolution === "Added") ||
      (filterEvolution === "Test Files Added" &&
        item.t_status_evolution === "Added") ||
      (filterEvolution === "Production Files Modified" &&
        item.p_status_evolution === "Modified") ||
      (filterEvolution === "Test Files Modified" &&
        item.t_status_evolution === "Modified");
    const matchesSearch =
      item.p_path.toLowerCase().includes(searchEvolution.toLowerCase()) ||
      item.t_path.toLowerCase().includes(searchEvolution.toLowerCase()) ||
      item.p_status_evolution
        .toLowerCase()
        .includes(searchEvolution.toLowerCase()) ||
      item.t_status_evolution
        .toLowerCase()
        .includes(searchEvolution.toLowerCase()) ||
  ...
  const currentEvolution = filteredEvolution.slice(
    indexOfFirstEvolution,
    indexOfLastEvolution
  );

  const evolutionTotalPages = Math.ceil(
    filteredEvolution.length / EVOLUTION_PER_PAGE
  );
  ...
  const handleShareConsentChange = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(
          `Failed to update consent: ${errorData.detail || response.statusText}`
        );
      ...
      if (newConsent) {
        toast.success(
          "You have agreed to share your results with the HighLight Test community."
        );
      } else {
  ...
  const fetchEvolutionData = async (revision) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/dashboards/general/co_evolution_details`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const openMaintenanceModal = async () => {
    ...
    if (maintenanceData.length === 0) {
      // Verifica se maintenanceData é um array vazio
  ...
  const fetchMaintenanceData = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/dashboards/general/maintenance_activities_details`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const maintenanceTotalPages = Math.ceil(
    maintenanceData.length / MAINTENANCE_PER_PAGE
  );
  ...
  const openDevelopersModal = async () => {
    ...
    if (developersData.length === 0) {
      // Verifica se developersData é um array vazio
  ...
  const fetchDevelopersData = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/dashboards/general/developers`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const developersTotalPages = Math.ceil(
    developersData.length / DEVELOPERS_PER_PAGE
  );
  ...
  const openCommitsModal = async () => {
    ...
    if (commitsData.length === 0) {
      // Verifica se commitsData é um array vazio
  ...
  const fetchCommitsData = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/dashboards/general/commits`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        const errorData = await response.json();
        toast.error(
          `Failed to fetch commits: ${errorData.detail || response.statusText}`
        );
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const openModal = async () => {
    ...
    if (modalData.length === 0) {
      // Verifica se modalData é um array vazio
  ...
  const fetchModalData = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/dashboards/general/code_distribution_details`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const filteredItems = modalData.filter((item) => {
    const matchesFilter =
      filter === "all" ||
      (filter === "true"
        ? item.is_test_file === true
        : item.is_test_file === false);
    const matchesSearch = item.file_path
      .toLowerCase()
              ...
              <>
                ...
                <nav>
                  <ul className="pagination justify-content-center">
                    {Array.from(
                      { length: developersTotalPages },
                      (_, index) => (
                        <li
                          key={index + 1}
                          className={`page-item ${
                            developersCurrentPage === index + 1 ? "active" : ""
                          }`}
                  ...
                  <ul className="pagination justify-content-center">
                    {Array.from({ length: commitsTotalPages }, (_, index) => (
                      <li
                        key={index + 1}
                        className={`page-item ${
                          commitsCurrentPage === index + 1 ? "active" : ""
                        }`}
              ...
              {indicator4 && (
                <div>
                  ...
                  <p>
                    ...
                    {indicator4?.pearson_correlation?.toFixed(4)}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="card-body">
              ...
              {indicator2 && (
                <Line
                  data={{
                    labels: Array.from(
                      { length: indicator2?.revisions?.length },
                      (_, i) => `${indicator2.revisions[i]}`
                    ),
                    datasets: [
                      {
        ...
        >
          ...
          <div className="modal-body">
            ...
            <div className="alert alert-info text-center">
              Detailed file-level information in the dataset analysis may not be
              available at the time of access, as it may still be processing.
            </div>
                      ...
                      datasets: [
                        {
                          ...
                          data: evolutionData
                            .filter(
                              (item) => item.p_status_evolution === "Added"
                            )
                            .map((item, index) => ({
                              x: item.revision,
                              y: 2 * index + 1,
                        ...
                        },
                        {
                          ...
                          data: evolutionData
                            .filter(
                              (item) => item.p_status_evolution === "Modified"
                            )
                            .map((item, index) => ({
                              x: item.revision,
                              y: 2 * index + 1,
                        ...
                        },
                        {
                          ...
                          data: evolutionData
                            .filter(
                              (item) => item.t_status_evolution === "Added"
                            )
                            .map((item, index) => ({
                              x: item.revision,
                              y: 2 * (index + 1),
                        ...
                        },
                        {
                          ...
                          data: evolutionData
                            .filter(
                              (item) => item.t_status_evolution === "Modified"
                            )
                            .map((item, index) => ({
                              x: item.revision,
                              y: 2 * (index + 1),
                      ...
                      scales: {
                        x: {
                          ...
                          ...(evolutionData.length > 0 &&
                            new Set(evolutionData.map((item) => item.revision))
                              .size === 1 && {
                              min: -0.5,
                              max: 0.5,
                              offset: true,
                  ...
                  <div>
                    <div className="mb-3">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setFilterEvolution("Production Files Added")
                        }
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          setFilterEvolution("Production Files Modified")
                        }
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          setFilterEvolution("Test Files Modified")
                        }
                      >
                        Test Files Modified
                      </button>
                    ...
                    <table className="table table-striped">
                      ...
                      <tfoot>
                        <tr>
                          <td colSpan="5" className="text-end">
                            <strong>
                              Total of {filteredEvolution.length} items
                            </strong>
                          </td>
                        </tr>
                      </tfoot>
                    ...
                    <nav>
                      <ul className="pagination justify-content-center">
                        {Array.from(
                          { length: evolutionTotalPages },
                          (_, index) => (
                            <li
                              key={index + 1}
                              className={`page-item ${
                                evolutionCurrentPage === index + 1
                                  ? "active"
                                  : ""
                              }`}
                              onClick={() =>
                                handleEvolutionPageChange(index + 1)
                              }
                            >
                              <button className="page-link">{index + 1}</button>
                            </li>
              ...
              <>
                <ul className="nav nav-tabs">
                  <li className="nav-item">
                    <button
                      className={`nav-link ${
                        coEvolutionActiveTab === "scatter" ? "active" : ""
                      }`}
                  ...
                  <li className="nav-item">
                    <button
                      className={`nav-link ${
                        coEvolutionActiveTab === "table" ? "active" : ""
                      }`}
                  ...
                  <div>
                    <div className="mb-3">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setCoEvolutionFilter("Detected Co-evolution")
                        }
                      >
                      ...
                      <button
                        className={`btn ${
                          coEvolutionFilter === "Without Co-evolution"
                            ? "btn-primary"
                            : "btn-outline-primary"
                        }`}
                        onClick={() =>
                          setCoEvolutionFilter("Without Co-evolution")
                        }
                      >
                      ...
                      <tfoot>
                        <tr>
                          <td colSpan="4" className="text-end">
                            <strong>
                              Total of {filteredCoEvolution.length} items
                            </strong>
                          </td>
                        </tr>
                      </tfoot>
                    ...
                    <nav>
                      <ul className="pagination justify-content-center">
                        {Array.from(
                          { length: coEvolutionTotalPages },
                          (_, index) => (
                            <li
                              key={index + 1}
                              className={`page-item ${
                                coEvolutionCurrentPage === index + 1
                                  ? "active"
                                  : ""
                              }`}
                              onClick={() =>
                                handleCoEvolutionPageChange(index + 1)
                              }
                            >
                              <button className="page-link">{index + 1}</button>
                            </li>
                    ...
                    datasets: [
                      {
                        label: "Lines of Code (%)",
                        data: [
                          indicator1.production_code,
                          indicator1.test_code,
                        ],
            ...
            {isLoading ? (
              ...
              <>
                <div className="mb-3">
                  ...
                  <button
                    className={`btn ${
                      filter === "false" ? "btn-primary" : "btn-outline-primary"
                    }`}
                  ...
                  <button
                    className={`btn ${
                      filter === "true" ? "btn-primary" : "btn-outline-primary"
                    } me-2`}
                    ...
                    {Array.from(
                      ...
                      (_, index) => (
                        <li
                          key={index + 1}
                          className={`page-item ${
                            maintenanceCurrentPage === index + 1 ? "active" : ""
                          }`}
        ...
        <div className="col-md-12 mb-6">
          <div className="card shadow-sm">
            <div className="card-body">
              <h5 className="card-title">
                {" "}
                {indicator3 && indicator3.cluster_name} Patterns Identified
              </h5>
              {indicator3 && (
        ...
        <div className="row">
          <div className="mt-5 p-4 border rounded bg-light">
            <h3 className="text-center">
              Share Your Results with the HighLight Test Community
            </h3>
            <p className="text-muted mt-3">
              By sharing your results, you contribute to the improvement of
              software development practices. Your data will be used by
              researchers, managers, and developers to better understand the
              co-evolution of test and production code. All sensitive data will
              not be shared and will not violate any principles of the{" "}
              <strong>General Data Protection Law (LGPD)</strong>.
            </p>
            <p className="text-muted">
              You can revoke your consent at any time, and your data will no
              longer be shared with the community.
            </p>
