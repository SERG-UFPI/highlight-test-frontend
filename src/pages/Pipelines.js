...
function Pipelines() {
    ...
    useEffect(() => {
        ...
        const fetchPipelines = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/crud/pipeline/repository/${repository_id}`, {
                    headers: {
                    'Authorization': `Bearer ${token}`,
                    }
                });
    ...
    const handleNewPipeline = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                toast.error(`Failed to create pipeline: ${errorData.detail || response.statusText}`);
                throw new Error(`HTTP error! status: ${response.status}`);
            }
    ...
    return (
        <div className="container mt-5">
            <ToastContainer />
            <p className="text-muted">Repository: <a href={repository.clone_url} target="_blank" rel="noopener noreferrer">{repository.clone_url}</a></p>
                ...
                {pipelines.items.map((pipeline) => (
                    ...
                    <div className="stages d-flex gap-2">
                        {(() => {
                            ...
                            { name: "Generate Time Series", key: "GENERATE_TIME_SERIES" },
                            { name: "Calculate Metrics", key: "CALCULATE_METRICS" },
                            { name: "Co-evolution Analysis", key: "CO_EVOLUTION_ANALYSIS" },
                            ];

                            const stageIndex = stages.findIndex((stage) => stage.name === pipeline.stage);

                            // Renderiza os stages com base no status
                            return stages.map((stage, index) => {
                            const isCompleted = index < stageIndex || index === stageIndex && pipeline.status === "Completed";
                            const isInProgress = index === stageIndex && pipeline.status === "In progress";
                            const isError = index === stageIndex && pipeline.status === "Error";
                    ...
                    <td>
                        {pipeline.stage === "Co-evolution Analysis" && pipeline.status === "Completed" ? (
                            <Link to={`/pipelines/${pipeline.id}`} className="btn btn-primary btn-sm">
                                View Details
                            </Link>
                        ) : (
